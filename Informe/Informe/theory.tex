...
\begin{equation}
    \frac{dP}{dT} = \frac{L_v P}{R_v T^2}
\end{equation}
donde \(L_v\) es el calor latente de vaporización y \(R_v\) es la constante específica del vapor de agua. Esta ecuación implica que la capacidad del aire para contener vapor de agua aumenta exponencialmente con la temperatura. En un clima más cálido, la atmósfera puede retener más humedad, lo que intensifica los eventos extremos de precipitación si los mecanismos de ascenso están presentes. La condensación del vapor sobre núcleos de condensación de nubes (CCN) da lugar a las gotitas de nube, cuyo tamaño es insuficiente para precipitar por gravedad \citep{feynman1963thermodynamics, pippard1964elements}.

\subsection{Procesos de colisión-coalescencia y Bergeron-Findeisen}
La formación de gotas de lluvia requiere el crecimiento de las partículas de condensación de nube hasta alcanzar un tamaño crítico. En nubes cálidas (temperatura \(> 0^\circ\)C), predomina el proceso de \textbf{colisión-coalescencia}, donde gotas de diferente tamaño y velocidad terminal colisionan y se fusionan. En nubes frías o mixtas, opera el \textbf{proceso de Bergeron-Findeisen}, relacionada con la diferencia de presión de vapor de saturación sobre el agua líquida y el hielo. En un ambiente mixto saturado respecto al agua, existe sobresaturación respecto al hielo, lo que induce la deposición de vapor sobre los cristales de hielo a expensas de las partículas subfundidas \citep{SupersaturationofWaterVaporinClouds}.

\subsection{Ascenso de masas de aire}
Para que se produzca precipitación significativa, es indispensable el ascenso de masas de aire húmedo que enfríen la parcela hasta la saturación \cite{houze2014cloud, pidwirny2006cloud, nws_cloud_development}. Los mecanismos de ascenso principales son:
\begin{itemize}
    \item \textbf{Convección libre:} Calentamiento diferencial de la superficie que genera burbujas de aire cálido menos denso que ascienden \citep{houze2014cloud, geerts2002convective, pidwirny2006cloud}.
    \item \textbf{Ascenso orográfico:} El viento horizontal se aproxima a una barrera montañosa y es forzado a elevarse, enfriándose adiabáticamente. Este mecanismo es crítico en la Bocacosta guatemalteca \citep{houze2014cloud, atmosferica1986viento, herrera2017descripcion, whiteman2000mountain}.
    \item \textbf{Convergencia en niveles bajos:} La fricción superficial o la dinámica de ondas del este generan zonas de acumulación de masa que fuerzan el movimiento vertical ascendente \citep{houze2014cloud, pidwirny2006cloud, makgabutlane_convergence_metservice}.
    \item \textbf{Ascenso frontal:} Superficies de discontinuidad entre masas de aire de distinta densidad generan mecanismos de ascenso \citep{houze2014cloud, nws_cloud_development}.
\end{itemize}
...
La SST es la variable oceánica más influyente en el clima tropical. Anomalías cálidas en la SST incrementan el flujo de calor latente y sensible hacia la atmósfera, reduciendo la estabilidad estática e incrementando la humedad en la capa límite. En el contexto del Pacífico Oriental, la SST modula la intensidad de la Zona de Convergencia Intertropical (ZCIT) y la convección asociada a ondas del este \citep{iglesias2010temperatura, cabos2002influencia, neelin1998enso}.

\subsection{Humedad relativa y específica}
La humedad específica (\(q\)) en niveles medios (500 hPa) es un indicador directo de la materia prima disponible para la convección. La humedad relativa (\(HR\)) proporciona información sobre qué tan cerca de la saturación se encuentra una columna atmosférica, siendo crucial para activar la convección \citep{pierrehumbert2007relative, peixoto1996climatology, trenberth2003changing, frei1998heavy}. 

\subsection{Viento (componentes zonal y meridional)}
El viento zonal (\(u\)) en niveles bajos (850 hPa) es fundamental para el transporte de humedad desde las fuentes oceánicas (el Jet de Bajo Nivel del Caribe o el flujo del Pacífico). La componente meridional (\(v\)) está asociada a la convergencia/divergencia y a la posición de la ZCIT. La cizalladura vertical del viento puede tanto inhibir como organizar los sistemas convectivos \citep{houze2014cloud, yim2014comparison, sodemann2013moisture, duran2010revision, raymond2006dynamics}.

\subsection{Geopotencial}
La altura geopotencial (\(Z\)) en niveles de 500 hPa permite identificar la presencia de vaguadas y dorsales en la tropósfera media. Una vaguada profunda sobre Centroamérica se asocia con ascenso de gran escala y enfriamiento en la tropósfera superior, lo que desestabiliza la columna atmosférica y favorece tormentas \citep{houze2014cloud, raible2003precipitation}.
...
\begin{itemize}
    \item \textbf{ENSO (El Niño-Southern Oscillation):} La fase cálida (El Niño) se asocia típicamente con déficit de precipitación en la vertiente del Pacífico guatemalteco debido al debilitamiento de los vientos alisios y al desplazamiento de la convección profunda hacia el Pacífico Central. La fase fría (La Niña) se correlaciona con excesos de lluvia \citep{raible2003precipitation, dai2000global, van2005searching, sun2015global}.
    \item \textbf{AMO (Atlantic Multidecadal Oscillation):} Afecta la SST del Atlántico Norte y modula la fuerza del transporte de humedad desde el Caribe hacia el Pacífico a través del istmo \citep{lin2022atlantic, wyatt2012atlantic}.
    \item \textbf{TNA (Tropical North Atlantic Index):} Anomalías cálidas en el Atlántico tropical intensifican la corriente de humedad hacia Centroamérica, compitiendo con la influencia del Pacífico \citep{maldonado2017early, perez2021combined}.
\end{itemize}
...
\subsection{Temperatura a 2 metros de la superficie}
...
Una red neuronal artificial es un modelo computacional inspirado en el sistema nervioso biológico, compuesto por unidades de procesamiento interconectadas llamadas neuronas \citep{salas2004redes, izaurieta2000redes}. Formalmente, una neurona artificial recibe un vector de entrada \(\mathbf{x} = (x_1, x_2, ..., x_n)\), calcula una suma ponderada de estas entradas y aplica una función de activación no lineal \(g(\cdot)\):
\begin{equation}
    y = g\left( \sum_{i=1}^{n} w_i x_i + b \right) = g(\mathbf{w}^T \mathbf{x} + b)
\end{equation}
donde \(\mathbf{w}\) son los pesos sinápticos (parámetros ajustables que determinan la importancia de cada entrada) y \(b\) es el sesgo o \textit{bias} \citep{ellacott2012mathematics}.
...
El entrenamiento de RNNs mediante retropropagación a través del tiempo (BPTT) implica el cálculo de gradientes a través de múltiples pasos temporales. Debido a multiplicaciones repetidas de la matriz \(\mathbf{W}_{hh}\) y las derivadas de las funciones de activación, los gradientes tienden a \textbf{desvanecerse} (tender a cero) o \textbf{explotar} (tender a infinito) a medida que los pasos de tiempo aumentan. En el contexto climático, donde la precipitación actual puede estar influenciada por la SST de varios meses atrás (teleconexiones ENSO), la RNN simple es incapaz de capturar estas dependencias de \textbf{largo plazo} debido al gradiente evanescente \citep{VanishingGradient}.

\section{Redes LSTM (Long Short-Term Memory)}

\subsection{Justificación de su uso}
La arquitectura LSTM fue diseñada específicamente para superar el problema del gradiente evanescente y aprender dependencias temporales de largo alcance. Dado que los fenómenos atmosféricos operan en múltiples escalas temporales (desde pequeños cambios locales en la altura geopotencial a una presión específica hasta la oscilación interanual del ENSO), la LSTM es la elección óptima para modelar la precipitación acumulada. Su capacidad para recordar y olvidar información selectivamente permite a la red mantener un estado que refleje la condición climática de fondo (fase de La Niña, Niño o Neutro) mientras reacciona a perturbaciones sinópticas de corta duración \citep{koreanLSTM, alonso2024mathematics}.

\subsection{Arquitectura interna y ecuaciones fundamentales}
La celda LSTM reemplaza la neurona recurrente simple por un módulo que contiene una \textbf{celda de estado} (\(C_t\)) y tres compuertas que regulan el flujo de información \citep{koreanLSTM}.
...
Mide la desviación estándar de los residuos (errores de predicción). Penaliza fuertemente los errores grandes debido al término cuadrático (llegando a valores altos cuando se presentan eventos extremos). Es útil para cuantificar la magnitud media del error en las mismas unidades que la variable observada (mm/día) \citep{HYNDMAN2006679}.
\begin{equation}
    RMSE = \sqrt{\frac{1}{n} \sum_{i=1}^{n} (y_i - \hat{y}_i)^2}
\end{equation}

\subsection{MAE (Error Absoluto Medio)}
Mide la magnitud promedio de los errores en un conjunto de predicciones, sin considerar la dirección. Es menos sensible a valores atípicos extremos que el RMSE \citep{HYNDMAN2006679}.
\begin{equation}
    MAE = \frac{1}{n} \sum_{i=1}^{n} |y_i - \hat{y}_i|
\end{equation}

\subsection{Correlación de Pearson (\(r\))}
Evalúa la relación lineal entre los valores predichos (\(\hat{y}\)) y los observados (\(y\)). Un valor cercano a 1 indica que el modelo captura correctamente la fase y la tendencia de la variabilidad de la precipitación, aunque pudiera tener un sesgo sistemático en la magnitud \citep{lalinde2018uso}.
\begin{equation}
...
Es un indicador de la bondad de ajuste de un modelo, utilizado comúnmente en las ciencias hidrológicas. Creado con el objetivo de mejorar las métricas ampliamente utilizadas, como el coeficiente de determinación y el coeficiente de eficiencia de Nash-Sutcliffe. Un valor de 1 indica una coincidencia perfecta entre el modelo y los datos observados. Un valor entre $1-\sqrt{2}$ y 1 indica valores aceptables, donde una mejor precisión se asocia a valores cercanos a 1. Un valor de NSE igual que $1-\sqrt{2}$ indica que la capacidad predictiva del modelo es igual a la de la media de los datos observados. Y un valor menor a $1-\sqrt{2}$ indica que la media es un mejor predictor que el modelo propuesto. Se define como:
\begin{equation}
    KGE = 1 -\sqrt{(r-1)^2+(\alpha-1)^2+(\beta-1)^2}.
\end{equation}

Donde $r$ es el coeficiente de correlación de Pearson, $\alpha$ representa la variabilidad de los errores de predicción, esto es, $\alpha=\sigma_s/\sigma_o$, donde $\sigma_s^2$ es la varianza de la serie temporal simulada y $\sigma_o^2$ es la varianza de la serie temporal observada. Finalmente $\beta$ es el sesgo del modelo, entendido como $\beta=\mu_s/\mu_o$, siendo $\mu_s$ la media de los datos simulados y $\mu_o$ la media de los datos observados \citep{Kling-Gupta}.
...
\section{Método tradicional de pronóstico}

\subsection{Metodología NextGen para Pronóstico Estacional}

\noindent La metodología NextGen (Next Generation Seasonal Forecast System por sus siglas en inglés) es un enfoque sistemático y flexible para el diseño, implementación, producción y verificación de pronósticos climáticos desarrollado por el Instituto Internacional de Investigación para el Clima y la Sociedad (IRI) de la Universidad de Columbia. Esta metodología ha sido adoptada por numerosos Servicios Meteorológicos e Hidrológicos Nacionales a lo largo de todo el mundo En Guatemala, el Instituto Nacional de Sismología, Vulcanología, Meteorología e Hidrología (INSIVUMEH) utiliza esta metodología para la generación de sus pronósticos climáticos de precipitación \citep{malawi}.

\subsubsection{Fundamentos}

El enfoque NextGen centra el diseño del sistema de pronóstico en las necesidades del usuario. En primer lugar, el usuario define la variable de interés (predictando), esto determina los procesos físicos que determinan su comportamiento, encontrando los predictores adecuados. Este mismo flujo de trabajo dicta las métricas a evaluar, y el tipo de pronóstico, por ejemplo, probabilístico o determinista \citep{irinext}.

\subsubsection{Base técnica}

El enfoque aprovecha la disponibilidad de datos en tiempo real de proyectos de conjuntos de múltiples modelos como el North American Multimodel Ensemble (NMME) de la NOAA y el Copernicus Climate Change Service de la Unión Europea \citep{cirohnext}. La metodología consta de varios componentes clave que la diferencian de aproximaciones convencionales \citep{irinext, munoznext, malawi}:
\begin{itemize}
	\item NextGen utiliza directamente los pronósticos estacionales de precipitación de los Modelos de Circulación General (GCM).
	
	\item Los pronósticos generados por los GCM presentan sesgos. Para corregirlos, NextGen aplica el enfoque de \textit{Model Output Statistics} (MOS), que utiliza estadísticas de las salidas de los modelos durante un período de (hindcast) para recalibrar las predicciones.
	
	\item NextGen enfatiza la combinación de pronósticos de múltiples GCM, una estrategia establecida para aumentar la destreza de los pronósticos estacionales.
	
	\item NextGen tiene la capacidad de proporcionar probabilidades de exceder umbrales específicos de interés para la toma de decisiones. Esto permite pronosticar no solo los valores medios, sino también los eventos extremos.
\end{itemize}

La metodología NextGen representa un avance en los sistemas de pronóstico climático operacional, al combinar las necesidades del usuario, herramientas estadísticas sofisticadas y modelos climáticos globales, proporcionando una base robusta y verificable para la toma de decisiones en sectores como la agricultura, la gestión de recursos hídricos y la prevención de desastres \citep{ponsnext}.
