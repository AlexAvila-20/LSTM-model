...
Uso:
    ...
    python diagnosticos.py mi_prediccion.nc
    python diagnosticos.py mi_prediccion.nc --save
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Genera diagnósticos completos de predicción vs observación a partir de un NetCDF."
    )
    parser.add_argument(
        "archivo",
        nargs="?",
        default="modelo_pixel_predictions.nc",
        help="Archivo NetCDF de entrada (por defecto: modelo_pixel_predictions.nc)"
    )
    parser.add_argument(
        "--save",
        action="store_true",
        help="Guardar figuras en PNG en lugar de mostrarlas en pantalla"
    )
    return parser.parse_args()


def guardar_o_mostrar(fig, nombre, guardar, outdir):
    if guardar:
...
def main():
    ...
    # Detectar nombres de dimensiones tiempo/lat/lon
    time_dim = [d for d in obs_da.dims if d in ['time', 'T', 'time_counter']][0]
    lat_dims = [d for d in obs_da.dims if d in ['lat', 'Y', 'y', 'latitude']]
    lon_dims = [d for d in obs_da.dims if d in ['lon', 'X', 'x', 'longitude']]
    
    obs_da = obs_da.rename({time_dim: 'time'})
    if lat_dims: obs_da = obs_da.rename({lat_dims[0]: 'lat'})
    if lon_dims: obs_da = obs_da.rename({lon_dims[0]: 'lon'})
    
    pred_time_dim = [d for d in pred_da.dims if d in ['time', 'T', 'time_counter']][0]
    pred_lat_dims = [d for d in pred_da.dims if d in ['lat', 'Y', 'y', 'latitude', 'Y_2']]
    pred_lon_dims = [d for d in pred_da.dims if d in ['lon', 'X', 'x', 'longitude', 'X_2']]
    
    pred_da = pred_da.rename({pred_time_dim: 'time'})
    if pred_lat_dims: pred_da = pred_da.rename({pred_lat_dims[0]: 'lat'})
    if pred_lon_dims: pred_da = pred_da.rename({pred_lon_dims[0]: 'lon'})
    
    # ─── Interpolar a la misma grilla espacial si es necesario ───────────
    if obs_da.shape != pred_da.shape:
        print(f"  Nota: Formas diferentes (obs={obs_da.shape} vs pred={pred_da.shape})")
        print(f"  Interpolando 'predicted' a la grilla de 'observed'...\n")
        pred_da = pred_da.interp_like(obs_da, method='linear')
    ...
    ax.legend(fontsize=9)
    fig.tight_layout()
    guardar_o_mostrar(fig, "01_dispersion.png", GUARDAR, OUTDIR)
    ...
    bias_map.plot(ax=ax, cmap="RdBu_r", vmin=-lim, vmax=lim,
    ...
    fig.tight_layout()
    guardar_o_mostrar(fig, "03_mapas_metricas.png", GUARDAR, OUTDIR)
    ...
    ax.legend()
    fig.tight_layout()
    guardar_o_mostrar(fig, "05_ciclo_anual.png", GUARDAR, OUTDIR)
    ...
    for ax, col, color in zip(
        ...
        if col == "Bias":
            ax.axhline(0, color="k", lw=0.5)
    fig.tight_layout()
    guardar_o_mostrar(fig, "06_metricas_mes.png", GUARDAR, OUTDIR)
    ...
    ax.legend()
    fig.tight_layout()
    guardar_o_mostrar(fig, "07_metricas_anual.png", GUARDAR, OUTDIR)

    # =====================================================================
    # 11. ANÁLISIS DE EXTREMOS
    # =====================================================================
    ...
    ax.set_aspect("equal")
    fig.tight_layout()
    guardar_o_mostrar(fig, "10_qqplot.png", GUARDAR, OUTDIR)
    print()
    ...
    nse_da = xr.DataArray(nse_grid, coords=[obs_da.lat, obs_da.lon], dims=["lat", "lon"])
    kge_da = xr.DataArray(kge_grid, coords=[obs_da.lat, obs_da.lon], dims=["lat", "lon"])
    ...
    fig.tight_layout()
    guardar_o_mostrar(fig, "13_nse_kge_mapa.png", GUARDAR, OUTDIR)
    ...
    fig.tight_layout()
    guardar_o_mostrar(fig, "14_tendencia_error.png", GUARDAR, OUTDIR)
    ...
    for p in [10, 25, 50, 75, 90, 95]:
        obs_p = np.percentile(obs_flat, p)
        pred_p = np.percentile(pred_flat, p)
        ratio = pred_p / obs_p if obs_p != 0 else np.nan
        print(f"  P{p:2d}  obs={obs_p:8.2f}   pred={pred_p:8.2f}   ratio={ratio:.3f}")
    print()
